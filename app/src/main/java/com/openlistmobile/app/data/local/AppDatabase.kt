...
@Database(entities = [ServerProfile::class, DirectoryCache::class, TransferTask::class, SyncRule::class], version = 7, exportSchema = false)
...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        val MIGRATION_6_7 = object : Migration(6, 7) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    "ALTER TABLE `transfer_tasks` ADD COLUMN `profileId` INTEGER NOT NULL DEFAULT -1"
                )
            }
        }
    }
